...
export default class SourceMap {
  ...
  sources: any;
  lineCount: number;
  constructor(mappings?: any, sources?: any) {
    this.mappings = this.purifyMappings([].concat(mappings));
  ...
  async addMap(map, lineOffset = 0, columnOffset = 0) {
    if (!(map instanceof SourceMap) && map.version) {
      const consumer = await this.getConsumer(map);
  ...
  async extendSourceMap(original, extension) {
    ...
    extension.eachMapping(mapping => {
      const originalMapping = original.originalPositionFor({
  ...
  findClosestGenerated(line, column) {
    ...
    while (
      ...
      const mid = this.mappings[middleIndex].generated.line;
      if (line < mid) {
    ...
    const mapping = this.mappings[middleIndex];
  ...
  originalPositionFor(generatedPosition) {
    const index = this.findClosestGenerated(
...
function lineCounter(str: string) {
  let lines = 1;
